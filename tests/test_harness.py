"""The 130 adversarial cases, run by pytest as well as by the report.
...
def test_the_case_file_is_the_size_it_claims_to_be():
    assert len(CASES) == 130, "cases.jsonl and the README must agree on the count"
    assert len({c["id"] for c in CASES}) == 130, "duplicate case id"


@pytest.mark.parametrize("case", CASES, ids=[c["id"] for c in CASES])
def test_case(case):
    outcome, rule = run_eval.run_case(case)
    want = case["expect"]
